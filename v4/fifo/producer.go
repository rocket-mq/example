...
func main() {
	...
	p := producer.New(data.Endpoint, data.AccessKey, data.SecretKey, data.TopicGlobalFifo, data.InstanceId)

	for i := 0; i < 100; i++ {
		body := fmt.Sprintf("%s-%d", "wb", i)
		msg := message.New(
			body,
			message.WithTag("test-5-tag"),
			message.WithKey("test-5-key"),
			message.WithProperties(map[string]string{"time": time.Now().Format("20060102150405")}),
			message.WithShardingKey("fifo"),
		)
